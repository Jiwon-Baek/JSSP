...
#
from data import set_problem
from config import update_global_variables

dataset = Dataset()
set_problem(dataset.name, dataset.path)
NUM_ITERATION = 1000
num_solution = 50
# num_solution = dataset.n_solution
update_global_variables(dataset.n_machine, dataset.n_job, dataset.n_op)
...
for i in range(num_solution, num_solution+NUM_ITERATION):
    ...
    individual = Individual(dataset.n_machine, dataset.n_job, seq=seq)
